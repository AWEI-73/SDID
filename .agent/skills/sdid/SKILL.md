---
name: sdid
description: SDID 統一開發框架 — 從需求設計到程式碼交付的完整流程。支援兩條設計路線：Blueprint（大藍圖 5 輪對話）和 Task-Pipe（POC 漸進式細部設計），兩條路匯流到 implementation_plan 後共用 BUILD Phase 1-8。觸發詞：「SDID」「藍圖」「blueprint」「新專案」「開發」「build」「繼續」「POC」「Task-Pipe」「快速建」「練習」「小專案」「create project」「new project」「繼續開發」「跑 build」「自動開發」「一鍵開發」「sdid 小修」「quick fix」。
---
