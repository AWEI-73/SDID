...
Blueprint 和 Task-Pipe 是**設計輸入方式**的選擇，不是兩條獨立管道。Draft 完成後全部合流進同一套主流程。
...
使用者意圖
  ...
  └─ 需求開發（新功能、新專案、繼續開發）
        │
        ├─ 需求模糊 → Blueprint（必要，5 輪對話）→ draft_iter-N.md
        └─ 需求明確 → Task-Pipe（直接建 draft）→ draft_iter-N.md
              ↓ (兩路線在此合流)
  ┌─────────────────────────────────────────────────┐
...
```
Layer 0: Blueprint（需求模糊時必要；blueprint.md 文件本身可選）
