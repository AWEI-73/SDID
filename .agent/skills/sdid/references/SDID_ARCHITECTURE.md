...
| 2 | AC 驗收層 | 呼叫 ac-runner 執行 @GEMS-AC，CALC AC 全 PASS 才過，SKIP 不計入失敗 |
...
| `blueprint/contract-writer.cjs` | Contract 推導撰寫（v4 路線） |
| `blueprint/draft-to-plan.cjs` | Enhanced Draft → implementation_plan |
...
| Draft Gate | PLAN 前 | Draft 功能需求 + AC | `blueprint/v5/draft-gate.cjs` |
...
| Phase 2 Gate | BUILD Phase 2 | AC 驗收：@GEMS-AC 全 PASS | `ac-runner.cjs` |
...
*本說明書依 ARCHITECTURE.md v6.0 同步更新。有新機制、新 Gate、新規則，直接補到對應章節。*
