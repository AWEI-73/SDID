...
唯一設計入口為 **Blueprint**（5 輪對話）。需求越不清楚，輪次越完整；需求已明確時輪次可縮短，但不跳過。

```
需求模糊 → Blueprint 完整 5 輪對話 → draft_iter-N.md
需求明確 → Blueprint 縮短（快速確認實體/Story）→ draft_iter-N.md
「小修」「fix」「改一下」→ MICRO-FIX（旁路，不進 BUILD）
```
...
使用者意圖
  ...
  └─ 需求開發（新功能、新專案、繼續開發）
        ↓
  Blueprint（5 輪對話，需求明確時可縮短）
        ↓
  draft_iter-N.md → draft-gate
        ↓
  ┌─────────────────────────────────────────────────┐
...
v7 最大變更：引入 TDD 驗收機制，以 `@TEST` 替代舊的 `@GEMS-AC-*` 標籤族。

### 四層設計模型

```
Layer 0: Blueprint（必要；blueprint.md 文件本身可選）
...
Layer 2: Per-iter Contract（每個 iter 獨立，屬於 iter）
  .gems/iterations/iter-N/contract_iter-N.ts — TypeScript interfaces + STORY-ITEM + @TEST
    └── contract-gate v5 + @CONTRACT-LOCK 封版
    └── spec-to-plan.cjs → contract @CONTRACT → implementation_plan（機械轉換）
    └── BUILD Phase 1-4（task-pipe/runner.cjs）
    └── SCAN — 全專案掃描更新 functions.json
    └── VERIFY（blueprint-verify.cjs）— 比對 draft vs 實作
...
| 2 | TDD 驗收層 | 讀 contract_iter-N.ts 找 @TEST：有 → vitest --run（RED→GREEN）；無 → tsc --noEmit |
...
| Phase 2 Gate | BUILD Phase 2 | @TEST → vitest GREEN；無 → tsc --noEmit | `task-pipe/runner.cjs` |
...
| `iter-N/poc/requirement_spec_*`（v5 legacy） | 已退休，不再使用 |
...
└── .gems/
    ├── design/                   # 設計文件集中（v6+）
    │   ├── blueprint.md          # 可選文件（Blueprint 5輪對話本身必要）
...
> TDD 驗收改在 contract 層用 `@TEST: src/...test.ts` 標記，Phase 2 機械執行
