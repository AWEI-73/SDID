...
## PRESERVE / INFER 規則（分片邊界機械化，LLM 不得改寫 PRESERVE 欄位）

### 分片定義
- **iter = module**：每個 iter 對應一個模組
- **contract block = slice**：每個 `@CONTRACT:` block 是一個 slice
- **slice = plan task**：一個 slice 對應 plan 裡的一個 Task，不能合併也不能拆分

### PRESERVE（原樣帶入，不得改寫）

| contract 欄位 | plan 對應 | 說明 |
|--------------|----------|------|
| `@CONTRACT: Name \| P \| Type \| Story` | Task 標題 + Files 標頭 | 名稱不得改，Type 決定路徑邏輯 |
| `@TEST: path` | Step 1 路徑 + Step 2 bash | 路徑必須原樣，不能推斷 |
| `@RISK: description` | Task 開頭 **Risk** 標注 | 風險描述必須原樣出現 |
| `@GEMS-FLOW: steps` | Step 3 `[STEP]` 標記 | step 名稱和順序不得改 |
| `Behavior:` 每一行 | Step 1 每個 `it()` 描述文字 | 必須出現在 it() 字串中 |

> ⚠️ **違規**：把 PRESERVE 欄位「意思對但文字改掉」= 測試語意漂移，gate 將無法驗證。

### INFER（LLM 可自行生成）

| 內容 | 來源 |
|------|------|
| 實作檔路徑 | 從 blueprint 路由結構圖取，Type 決定子目錄 |
| 測試程式碼（it() body） | 從 Behavior: 行語意生成，具體 assertion |
| 實作程式碼 | 骨架推導，Phase 2 再修到 GREEN |
| GEMS 標籤 | 從 @CONTRACT 欄位組合（Name \| Priority \| StoryId \| FLOW） |

### @PLAN_TRACE（spec-to-plan.cjs 自動輸出，可追蹤來源）
