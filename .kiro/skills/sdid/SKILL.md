...
CYNEFIN-CHECK（強制）
    ↓ @PASS
TDD Contract Subagent（needsTest:true → RED 驗證 → AC 回填）
    ↓ READY
...
可選前置（需求模糊時）:
  ...
  BLUEPRINT-CONTINUE → 補 AC          → .gems/design/blueprint.md（續跑）
...
主流程（唯一路線）:
  ...
  CYNEFIN-CHECK（強制）
          ↓ @PASS
  TDD Contract Subagent（needsTest:true → RED 驗證 → 回填 AC）
          ↓ READY
  contract_iter-N.ts
...
```
Draft 完成 → CYNEFIN-CHECK → @PASS → TDD Contract Subagent（needsTest:true 的 action）
...
- 執行步驟：讀主藍圖 → 找 [CURRENT]/[STUB] iter → 補 AC + Demo Checkpoint → 產出 Stub Draft → Gate 驗證 → 存檔
...
| [tdd-contract-prompt.md](references/tdd-contract-prompt.md) | TDD Contract Subagent prompt（CYNEFIN @PASS 後驗證 AC 可測性） | CYNEFIN @PASS → CONTRACT 寫入前 |
