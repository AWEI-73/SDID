...
```
主流程（唯一路線）:
  Blueprint 5輪對話 → draft_iter-N.md → draft-gate → Contract → contract-gate
    → spec-to-plan → BUILD Phase 1-4 → SCAN → VERIFY → COMPLETE
```

> ⚠️ 重要：Blueprint 是必要設計階段，需求模糊時完整 5 輪，需求已明確時可縮短但不跳過。
> blueprint.md 文件本身可選（小專案可不存檔）；Draft 是強制產物。
> Cynefin 分析是 Blueprint 內部工具，不是獨立的 gate 或 phase。
...
| 2 | TDD 驗收層 | 讀 contract_iter-N.ts 找 @TEST 標籤：有 → vitest --run（測試在 contract 階段就寫好，Phase 1 RED，Phase 2 GREEN）；無 → tsc --noEmit（DB/UI 層只驗型別）|
...
draft_iter-N.md
  Story-X.Y: 功能需求描述
    │
    ├── draft-gate（Cynefin 分析為 Blueprint 內部工具，不是 gate）
    │
    contract_iter-N.ts
    │  TDD Contract Subagent：needsTest:true action
    │  → 寫測試檔（RED）
    │  → 加 @TEST: src/modules/.../xxx.test.ts
    │
    ...
    │   有 @TEST → vitest --run（修實作讓測試 GREEN，不能動測試檔）
    │   無 @TEST → tsc --noEmit（DB/UI 層只驗型別）
    │
    └── BUILD Phase 4 / SCAN
...
> `// AC-X.Y` 已 deprecated（v7.0），由 contract.ts 的 `@TEST` 取代。
