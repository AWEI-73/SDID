...
`;
  ...
  // 檢查是否有現有函式快照 (pointer only，不塞 context)
  const functionsJsonPath = path.join(target, '.gems', 'docs', 'functions.json');
  let existingFnsPointer = '';
  if (fs.existsSync(functionsJsonPath)) {
    try {
      const raw = JSON.parse(fs.readFileSync(functionsJsonPath, 'utf8'));
      const count = (raw.functions || raw).length || 0;
      if (count > 0) {
        const relFunctionsPath = path.relative(process.cwd(), functionsJsonPath).replace(/\\/g, '/');
        existingFnsPointer = `\n@FUNCTIONS_SNAPSHOT (${count} 個) → 讀 ${relFunctionsPath}`;
      }
    } catch { /* skip */ }
  }
...
檢查: ${issues.length ? '[NEEDS CLARIFICATION] 需求不夠清晰' : 'OK 通過'}${existingFnsPointer}

@RULES
- 禁止腦補：模糊就問，不要猜
- 必須釐清：使用者角色、核心目標、資料結構
- 邏輯預檢：生命週期、邊界條件、依賴方向
- 設計新功能前先讀 @FUNCTIONS_SNAPSHOT，避免重複實作`);
