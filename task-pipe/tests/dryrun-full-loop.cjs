 ...
 *   2. Blueprint Flow 狀態轉移: NO_DRAFT → GATE → CYNEFIN → CONTRACT → PLAN → BUILD 1-4 → SCAN → VERIFY → COMPLETE
 *   3. Milestone 功能驗證: M24(log精簡) M26(CYNEFIN quick-scan) M25(adversarial hint) M17(@UNTAGGED) M23(subagent hint) M16(API簽名)
...
// 1.5 inferStateFromLogs — gate-check-pass → CYNEFIN_CHECK
{
  const proj = mkTmp('state-cynefin');
  writeLog(proj, 1, 'gate-check-pass');
  const state = stateMachine.inferStateFromLogs(proj, 1, [], []);
  assert('inferStateFromLogs: gate-check-pass → CYNEFIN_CHECK', state?.phase === 'CYNEFIN_CHECK', `got: ${JSON.stringify(state)}`);
  fs.rmSync(proj, { recursive: true, force: true });
}

// 1.6 inferStateFromLogs — gate-check-pass + cynefin-check-pass → CONTRACT
{
  const proj = mkTmp('state-contract');
  writeLog(proj, 1, 'gate-check-pass');
  writeLog(proj, 1, 'cynefin-check-pass');
  const state = stateMachine.inferStateFromLogs(proj, 1, [], []);
  assert('inferStateFromLogs: cynefin-pass → CONTRACT', state?.phase === 'CONTRACT', `got: ${JSON.stringify(state)}`);
  fs.rmSync(proj, { recursive: true, force: true });
}

// 1.7 inferStateFromLogs — contract-pass → PLAN
{
